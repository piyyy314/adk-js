...
## 2025-06-16 - Centralizing Cancellation Feedback

**Learning:** Using a centralized `handleCancellation` type guard in `@clack/prompts` ensures that the 'outro' message is consistently displayed whenever a user cancels an interactive prompt, while also maintaining TypeScript type safety for subsequent logic. Proactively providing URLs for external setup (like Google AI Studio) right before the related prompt significantly reduces friction in the onboarding flow.
**Action:** Implement a `handleCancellation` helper in interactive CLI tools and provide actionable links for external dependencies immediately before they are requested.
