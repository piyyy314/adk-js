...
export async function createAgent(options: AgentCreationOptions) {
  ...
  if (!options.apiKey && !options.project) {
    ...
    if (backend === 'vertex') {
      ...
      if (!options.forceYes) {
        log.info('Get your API Key from https://aistudio.google.com/');
      }
  ...
  if (!options.forceYes) {
    note(
      `Created the following files in ${agentDir}:\n` +
        ...
        `\n\nRun 'cd ${options.agentName}' and use one of the following commands to start the agent:\n` +
        `  - npm run web: Start the agent in a web interface\n` +
        `  - npm run cli: Start the agent in an interactive CLI`,
