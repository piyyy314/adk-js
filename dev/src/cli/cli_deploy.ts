...
export async function deployToCloudRun(options: DeployToCloudRunOptions) {
  ...
  try {
    ...
    if (await isFolderExists(options.tempFolder)) {
      s?.message('Cleaning up existing temporary files...');
      await fs.rm(options.tempFolder, {recursive: true, force: true});
    }

    s?.message('Copying agent source files...');
    ...
    const cleanupSpinner = process.stdout.isTTY ? spinner() : null;
    cleanupSpinner?.start('Cleaning up temporary files...');
    await fs.rm(options.tempFolder, {recursive: true, force: true});
    await agentLoader.disposeAll();
    cleanupSpinner?.stop('Temporary files cleaned up.');
