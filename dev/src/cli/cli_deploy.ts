  ...
  try {
    ...
    if (!project || project === '(unset)') {
      if (process.stdout.isTTY) {
        const projectPromptResponse = await text({
        ...
        if (handleCancellation(projectPromptResponse)) {
          return;
        }
        project = projectPromptResponse;
      } else {
        throw new Error(
    ...
    if (!region) {
      if (process.stdout.isTTY) {
        const regionPromptResponse = await text({
        ...
        if (handleCancellation(regionPromptResponse)) {
          return;
        }
        region = regionPromptResponse;
      } else {
        throw new Error(
