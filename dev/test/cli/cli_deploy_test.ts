...
describe('deployToCloudRun', () => {
  ...
  it('should call intro and outro when TTY', async () => {
    await deployToCloudRun(defaultOptions);

    expect(intro).toHaveBeenCalledWith('Deployment to Cloud Run');
    expect(outro).toHaveBeenCalledWith('Deployment completed successfully!');
  });

  it('should NOT call intro and outro when NOT TTY', async () => {
    Object.defineProperty(process.stdout, 'isTTY', {
      value: false,
      configurable: true,
    });
